...
│   │   └── support-tickets/ Error-report support tickets: screenshot capture + upload, admin triage
│   ├── infrastructure/
...
│   │   └── storage/       S3Service (private-bucket upload + signed view URLs, see support-tickets)
│   └── shared/
...
### support-tickets (`src/modules/support-tickets/`, added 2026-09-04)
- Error-reporting feature: a screenshot + error context capture flow (frontend) that files a `SupportTicket` for Billinx's internal team to triage — auto-triggered on an unhandled crash, or manually via a floating "Report an issue" button on every authenticated dashboard page.
- `POST /v1/support-tickets` — multipart (`screenshot` file + `errorMessage`/`pageUrl`/`browserInfo` required, `stackTrace`/`userDescription` optional). Guarded by a new `OptionalJwtGuard` (`src/modules/identity/guards/optional-jwt.guard.ts`) — tries `JwtGuard`, but unlike it never rejects the request when the token is missing/invalid, so the same endpoint accepts both an authenticated dashboard caller and a genuinely pre-auth crash (e.g. on `/login`). `tenantId`/`userId` are **never read from the request body** — always re-derived server-side from the verified JWT context when one exists (`null`/`null` otherwise), so a caller can't spoof another tenant's ticket. Rate limited by a new `SupportTicketRateLimitGuard` (`src/shared/guards/support-ticket-rate-limit.guard.ts`, 10 requests / 10 min, bucketed by tenantId when authenticated else by IP) — specifically to stop an auto-capture-on-crash loop from becoming a spam/storage-cost vector. Screenshot content is verified by magic bytes (`file-type`, PNG/JPEG only), mirroring the `incoming-invoice` attachment pattern.
- Screenshot storage: new `S3Service` (`src/infrastructure/storage/`) — uploads to a **private** bucket (`SUPPORT_TICKETS_S3_BUCKET`, Block Public Access on, see `infra/modules/s3-support-tickets`), key `support-tickets/{tenantId|anonymous}/{ticketId}.{ext}`. The DB only ever stores the object key, never a public URL; a signed URL (15 min TTL) is generated on demand by `getSignedViewUrl()`, only from the admin detail endpoint and the internal notification email. Reuses the same "no credentials → skip real upload, log and continue" dev-safe pattern as `EmailService` (gated on `AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY`) so ticket creation never fails in an environment with no S3 access configured — the screenshot key is still stored, it just won't resolve to a real object until credentials are present.
- On creation, fires (fire-and-forget) `EmailService.sendSupportTicketNotification()` to `SUPPORT_TICKETS_NOTIFY_EMAIL` (default `eng@billinx.ng`) with a summary and a freshly-signed screenshot link. `errorMessage`/`pageUrl` are HTML-escaped before interpolation into that email — unlike every other template in `email.service.ts`, these two fields originate from the browser (a crash or a user's free-text report), not a trusted server value.
- Admin endpoints — reused **`AdminJwtGuard`** (`src/modules/admin/guards/admin-jwt.guard.ts`), the exact guard already applied to every other `v1/admin/*` route in this codebase (`admin.controller.ts`, `kyb.controller.ts`'s `admin/kyb/verify-cac`, `activity.controller.ts`'s `admin/activity`/`admin/errors`); no new guard was written. `GET /v1/admin/support-tickets` (filterable by `status`/`tenantId`, paginated), `GET .../:id` (includes a freshly-generated signed screenshot URL), `PATCH .../:id` (status only: `OPEN|IN_PROGRESS|RESOLVED`). All three go through `PrismaService.asAdmin()` (bypasses RLS), same as every other cross-tenant admin read in this codebase — necessary here since tickets can have a `null` tenantId (pre-auth crash) that no tenant-scoped RLS policy would ever match.
- `SupportTicket.tenantId`/`userId` are plain nullable string columns with no Prisma relation to `Tenant`/`User` — matching the existing `Notification` model's convention, not `RecurringInvoice`'s. RLS: `ENABLE`/`FORCE ROW LEVEL SECURITY` + `tenant_isolation` policy, matching the established pattern — but since `tenantId` is nullable here, a `null`-tenant row is (by design) invisible to every tenant-scoped query; only `asAdmin()` calls (pre-auth ticket creation, and all three admin endpoints) ever see those rows.
- Frontend (`apps/web`): `html2canvas` captures the viewport on trigger. `lib/errorReporting.ts` is the shared capture-and-upload pipeline, used by all three trigger paths: a root-level `ErrorBoundary` (React render errors, `app/layout.tsx`, wraps `AuthProvider`), `window.onerror`/`unhandledrejection` listeners (`GlobalErrorListeners`, also mounted in the root layout so pre-auth pages are covered), and the floating `ReportIssueButton` (mounted in `app/(dashboard)/layout.tsx` only — "visible on all authenticated pages" — with an optional free-text description field). Auto-capture has a 30s client-side cooldown to avoid hammering the backend rate limit during a crash loop; the manual button isn't subject to it. Upload is fire-and-forget in all three paths — the confirmation toast / boundary fallback UI never waits on the network round-trip. If `html2canvas` itself throws, a tiny inline 1×1 placeholder PNG is sent instead, so ticket creation never depends on capture succeeding (the backend's `screenshot` field is required).
- Admin UI: `app/(admin)/admin/support-tickets/page.tsx`, added to the admin sidebar nav — table (tenant, error, status, created) + a detail modal (stack trace, user note, screenshot via signed URL, status buttons), matching the existing admin list-page pattern (`admin/erasure`, `admin/consent`).
- Terraform: `infra/modules/s3-support-tickets` (private bucket, Block Public Access, SSE-S3, 180-day lifecycle expiration), wired into `infra/main.tf`. **Not yet applied — needs engineering review.** This app's existing convention (see `modules/secrets` — `AWS_SES_KEY`/`AWS_SES_SECRET`) is a manually-created, Terraform-unmanaged "SES-scoped IAM user" whose static keys are injected as `AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY`; `S3Service` reuses that same env var pair rather than an ECS task-role IAM policy, so that IAM user additionally needs `s3:PutObject`/`s3:GetObject` granted on the new bucket (module output `bucket_arn`) — the same manual step as its existing SES permissions, not something Terraform manages here.
...
| `SupportTicket` | Error-report ticket (added 2026-09-04): `tenantId`/`userId` nullable (pre-auth crashes), `screenshotKey` (private S3 object key, never a public URL), `errorMessage`/`stackTrace`/`pageUrl`/`browserInfo`/`userDescription`, `status` (`SupportTicketStatus`: `OPEN\|IN_PROGRESS\|RESOLVED`). See `support-tickets` module notes above. |
...
20260904220956_add_support_tickets  # feat/error-reporting — SupportTicket model (SupportTicketStatus enum); nullable tenantId (pre-auth crashes) — same RLS pattern as 20260721131342, applied to a nullable column for the first time (a null-tenant row is invisible to every tenant-scoped RLS query by design; only asAdmin() calls ever see it)
```

48 migrations applied as of 2026-09-04; database schema confirmed in sync via `npx prisma migrate status` (verified live against a local Postgres/Redis via docker-compose).
...
- 154+ PRs merged to main (through PR #188 as of 2026-07-09; several since, including `feat/docs` Swagger/OpenAPI coverage, `feat/auth` API key scopes + per-key rate limiting, `chore/ui-cleanup-admin-actions`, `feat/error-reporting` (support-tickets feature + the `s3-support-tickets` Terraform module — module written and planned, **not yet applied**, see the `support-tickets` module notes above), and `fix/optional-jwt-guard-narrowing` below)
- 973 tests passing, 61 suites, 45 DB models, 23 enums — up from 863/56 (2026-07-09) via several PRs since (OAuth/prettier/lint work not separately itemised here) plus `fix/nrs-schema-alignment` (2026-07-18): new `payment.service.spec.ts` (18 tests — this file, `src/modules/invoice/services/payment.service.ts`, had **zero** prior coverage), `update-status.worker.spec.ts` (5 tests), `interswitch.adapter.spec.ts` rewritten for OAuth (53 tests), plus new/updated cases in `invoice-validation.service.spec.ts`, `tenant.service.spec.ts`, and `product-catalog.service.spec.ts` for the new NRS-schema rules; plus `fix/jwt-name-claim` (1 test), `feat/pdf` (8 tests: new `invoice-pdf.service.spec.ts` — 7 tests, plus 1 new `invoice-dashboard.controller.spec.ts` case), and `feat/debug` (11 tests: 8 new `previewPayload` cases in `interswitch.adapter.spec.ts`, 3 new `invoice-dashboard.controller.spec.ts` cases) since. Backend test count is 993 as of `chore/ui-cleanup-admin-actions` (2026-07-18) — one delegation test for the retired dashboard `:id/xml` route was removed along with the route itself; no other suites changed. 1010 tests / 64 suites as of `fix/line-item-normalisation` (2026-07-20): new `invoice.service.spec.ts` (17 tests — `InvoiceService` had no dedicated spec file before this, only the indirect `invoice-flow.integration.spec.ts` coverage) for `normaliseLineItems()` and its five call sites, see the line-item shape-mismatch note above. **1062 tests / 67 suites** as of `feat/recurring-invoices` (2026-07-21): new `recurring-invoice.service.spec.ts` (44 tests), `recurring-invoice.controller.spec.ts` (7 tests), `recurring-invoice.scheduler.spec.ts` (1 test) — see the `invoice` module's recurring-invoices note above. Backend/frontend unit test counts unchanged by `test/e2e-playwright-tests` (2026-07-21) — that PR adds a *separate* suite, 7 Playwright tests (2 one-time auth-setup + 5 journeys) in `apps/web/e2e/`, run by their own `npm run test:e2e` / CI job rather than `npm test` — see the Test coverage section's E2E-tests note (just above the "Dead/misleading UI" heading) for the full design and the several real bugs it caught. **1078 tests / 70 suites** as of `feat/error-reporting` (2026-09-04): new `support-ticket.service.spec.ts` (6 tests) and `support-tickets-admin.controller.spec.ts` (6 tests) — see the `support-tickets` module notes above; plus `optional-jwt.guard.spec.ts` (4 tests), added when `OptionalJwtGuard`'s catch-all was narrowed to only treat `UnauthorizedException` (token-validation failures) as anonymous, rethrowing anything else instead of silently swallowing it.
- Last merged PRs: `fix/optional-jwt-guard-narrowing` (this one — narrows `OptionalJwtGuard` to only treat `UnauthorizedException` as anonymous, rethrowing everything else), `feat/error-reporting`, `test/e2e-playwright-tests`, `feat/recurring-invoices`, `chore/ui-cleanup-admin-actions`

### Open Issues

**Found 2026-09-05 during `feat/error-reporting` Terraform-plan verification (unfixed, out of scope — kept here so nobody re-discovers it from scratch):**
- **`infra/main.tf` has a circular dependency between `module.ecs` and `module.cloudwatch`.** `terraform plan -target=module.s3_support_tickets` against the real root config fails at graph-build time with `Error: Cycle` — `module.cloudwatch`'s alarms (`aws_cloudwatch_metric_alarm.ecs_cpu_high`/`ecs_memory_high`/`alb_5xx_high`) read `module.ecs`'s outputs (`cluster_name`, `service_name`, `task_definition_family`, etc.), while `module.ecs` reads `module.cloudwatch`'s `app_log_group_name`/`application_log_group_name` outputs for its task definition's log config — each module depends on the other's outputs, which Terraform's graph cannot resolve regardless of which resource is targeted. Confirmed live: plotting *any* `-target` against the real `infra/main.tf` hits this same cycle, since Terraform validates the whole graph before pruning to the target. Verified this is pre-existing and unrelated to the new `s3-support-tickets` module by re-running the same plan against an isolated throwaway root config containing only `module.s3_support_tickets` (clean, 5 resources to add, no errors). Not fixed here — resolving it means deciding which module should own the dependency edge (e.g. have `ecs` accept the log group *name* as a plain string variable computed independently, rather than an object import that's actually a full ARN roundtrip, or split the alarm resources that need ECS outputs into a separate pass) — a real infra-design call for engineering, not a mechanical fix, and touches the production `infra/main.tf` directly.
...
- Error reporting (2026-09-04): `ErrorBoundary` + `GlobalErrorListeners` (both mounted in the root `app/layout.tsx`, so pre-auth pages are covered too) auto-capture a screenshot and file a support ticket on any unhandled React render error, `window.onerror`, or `unhandledrejection`. A floating `ReportIssueButton` (`app/(dashboard)/layout.tsx` only) gives users a manual "Report an issue" trigger with an optional description field. New admin-only `app/(admin)/admin/support-tickets/page.tsx` (sidebar entry added) lists and triages the resulting tickets, including the screenshot via a signed URL. See the `support-tickets` module notes above for the full backend design.

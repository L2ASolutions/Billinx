...
describe("XmlInvoiceBuilder", () => {
  ...
  describe("build() — required fields", () => {
    ...
    it("maps CREDIT_NOTE to 381", () => {
      const xml = builder.build(makeInvoice({ invoiceTypeCode: "CREDIT_NOTE" }));
  ...
  describe("build() — optional fields", () => {
    ...
    it("includes DueDate when set", () => {
      const xml = builder.build(makeInvoice({ dueDate: new Date("2024-06-14") }));
      expect(xml).toContain("<DueDate>2024-06-14</DueDate>");
    });

    it("includes Note when set", () => {
      const xml = builder.build(makeInvoice({ note: "Payment due in 30 days" }));
    ...
    it("includes BusinessId when provided", () => {
      const xml = builder.build(makeInvoice(), "6dj03c76-1d83-4a39-a4de-51bd70547aef");
      expect(xml).toContain("<BusinessId>6dj03c76-1d83-4a39-a4de-51bd70547aef</BusinessId>");
    });
    ...
    it("renders multiple InvoiceLine elements for multiple line items", () => {
      const inv = makeInvoice({ lineItems: [LINE_ITEM, { ...LINE_ITEM, hsnCode: "CC-002" }] });
    ...
    it("renders PaymentMeans when present", () => {
      const inv = makeInvoice({
        paymentMeans: [{ paymentMeansCode: "30", paymentDueDate: "2024-06-14" }],
      });
      const xml = builder.build(inv);
    ...
    it("renders InvoiceDeliveryPeriod when present", () => {
      const inv = makeInvoice({
        invoiceDeliveryPeriod: { startDate: "2024-06-14", endDate: "2024-06-16" },
      });
      const xml = builder.build(inv);
  ...
  describe("parse() — round-trip from XML", () => {
    ...
    it("restores taxTotal array with subtotals", () => {
      ...
      expect(result.taxTotal[0].taxSubtotal[0].taxCategory.id).toBe("STANDARD_VAT");
    });
    ...
    it("restores postal address fields", () => {
      const xml = builder.build(makeInvoice());
      const result = builder.parse(xml);
      expect(result.seller.postalAddress.streetName).toBe("32 Owonikoko Street");

...
3. ~~**No CSID (Cryptographic Stamp ID) storage**~~ — **RESOLVED (investigated 2026-07-21, no code change).** This item's premise was wrong: confirmed against the actual `InterswitchAdapter` response handling and `docs/interswitch-api-spec.md`'s documented `postInvoice` response, the Interswitch API never returns a `csid` field at all — the response is `{ code, message, data: { IRN, PostingDateTime, QRCodeData } }`. Interswitch applies ECDSA signing at the Access Point Provider layer before submitting to NRS, and the returned `QRCodeData` encodes that cryptographic stamp. Billinx already stores and renders `qrCodeBase64` correctly (PDF and XML), which is the compliance-relevant artifact — there is no separate CSID value to capture. (The `SubmissionResult.csid` field referenced above is a vestigial, unused-in-production type field only ever populated by `MockAdapter`'s sandbox simulator, not by the real adapter.)
...
- SQL injection via raw Prisma queries — fixed with parameterised Prisma calls
- RLS enforcement gap — PrismaService middleware now sets `app.current_tenant_id` on every request
- RSA key pair for JWT signatures — SecretsService fetches RS256 keys from AWS Secrets Manager
- Fail-closed auth guard behaviour — guards throw 401/403 on any error
- Role-based access checks on sensitive admin endpoints — AdminJwtGuard enforced

**Audit Improvements**
- AuditLogInterceptor records every HTTP request/response (redacting sensitive fields)
- ActivityEvent model now stores SHA-256 hash chain (`entryHash`, `previousHash`)
- Chain verification available at `GET /v1/admin/audit/verify`
- `GET /v1/activity/export` returns CSV of all tenant activity events

**Security Hardening**
- CORS restricted to `ALLOWED_ORIGINS` env var (not wildcard)
- Helmet middleware applied globally
- Admin login lockout and rate limiting in place
- Webhook signing keys encrypted at rest (AES-256)
- All secrets in production come from AWS Secrets Manager

**Infrastructure**
- Terraform modules for VPC, ECS Fargate, RDS, ElastiCache, ALB, ECR, Secrets Manager
- GitHub Actions workflows: `deploy.yml` (push to main) and `pr-checks.yml` (PR linting/tests)
- Terraform remote state setup documented in `docs/terraform-state-setup.md`

**Developer Experience (Group 2)**
- Postman Collection v2.1 at `docs/billinx-api.postman_collection.json`
- Postman setup guide at `docs/postman-setup.md`
- Developer quickstart at `docs/quickstart.md` (6 steps, Nigeria e-invoicing data)
- API versioning and deprecation policy at `docs/api-changelog.md`
- `GET /openapi.json` endpoint for machine-readable OpenAPI spec (non-production)

**Data Retention (Group 3)**
- Invoice archiving after 7 years (daily cron at 02:00 UTC)
- ActivityEvent archiving after 2 years
- Admin endpoints: `GET /v1/admin/retention/stats`, `POST /v1/admin/retention/run`

**Immutable Audit Log (Group 4)**
- SHA-256 hash chaining on every ActivityEvent
- `GET /v1/admin/audit/verify` validates the entire chain for tampering

**Product Catalog (Group 5)**
- `src/modules/product-catalog/` with full CRUD
- `GET /v1/products/:id/as-line-item` for invoice line item formatting
- Tenant-scoped, search and filter support

**Compliance Export (Group 6)**
- CSV, JSON, and monthly report export for tenant invoices
- Platform-wide CSV export for admin FIRS audit submissions
- 60-second Redis rate limit per tenant per export

**Monitoring (Group 7)**
- Health check enhanced with DB latency, Redis latency, queue depth, uptime
- `GET /v1/admin/metrics` with today/week/month invoice stats and webhook delivery rates
- `GET /v1/admin/queue/status` for BullMQ job counts
- `POST /v1/admin/queue/retry-failed` for operational job recovery

### Updated Readiness Scores

| Category | Score | Change |
|---|---|---|
| **Overall** | ~88% | up from 61% |
| Security | 95% | SQL injection fixed, RLS live, RSA keys, fail-closed auth, role checks |
| Compliance/FIRS | 85% | XML support, all invoice types, submission flow tested |
| Production Readiness | 82% | Sentry, CORS, correlation IDs, Terraform state |
| Developer Experience | 90% | Postman collection, quickstart, OpenAPI export |
| Data Integrity | 85% | Hash-chained audit log, RLS enforcement |
| Observability | 88% | Sentry, health check with latency, metrics endpoint |

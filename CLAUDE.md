...
- **New, previously unflagged:** `dashboard/page.tsx` — **1,207 lines** (was documented as 1,111; corrected during this series' assessment pass, 2026-07-20 — not yet extracted, planned as PR 2). `invoices/[id]/page.tsx` planned as PR 3 (1,536 lines, was documented as 1,527). `purchases/page.tsx` — 851 lines. `payments/page.tsx` — 845 lines.

**Component extraction series (2026-07-20)** — three oversized dashboard pages assessed together, extracted as three separate PRs in ascending-risk order (`invoices/new/page.tsx` deliberately excluded — recently changed by the productCategory/serviceCategory PR, left to settle):
...
- PR 2 — `dashboard/page.tsx` — planned next. Will need shared `types.ts` and `visibility.ts` modules (unlike settings, `CustomizeSheet` independently consumes the same visibility helpers the page uses, so they can't just move into the page) plus care around Recharts tooltip typing (`content={Tooltip}` function-reference form) and the non-standard Bar/Pie `onClick` datum access — both must survive the move unchanged.
- PR 3 — `invoices/[id]/page.tsx` — planned last (highest line count, most modals/shared handlers triggered from multiple UI locations).

...
8. **BullMQ queues are constructed as module-level singletons that eagerly open a live Redis connection at import time** — found while writing webhook tests (a test hung indefinitely; root cause was importing `WebhookService` transitively opening a real, never-closed Redis connection via `new Queue(...)` at the top of `webhook.queue.ts`). **`webhook.queue.ts` fixed 2026-07-04** — the queue is now built lazily on first actual use, verified by importing `WebhookController` with the queue module *not* mocked and confirming a clean process exit. **The same pattern still exists, unfixed, in three other files** and wasn't touched here because their exported queue objects are read directly by other code, raising the blast radius beyond a queues-only change: `submission.queue.ts` (read directly by `health.controller.ts` — the endpoint AWS ECS/CI use to decide on deploy rollback — and by `admin.service.ts`), `bulk-submission.queue.ts` (read by `admin.service.ts`), and `invoice/vat-reminder.queue.ts` (read by the VAT reminder cron scheduler). Fixing those properly means either the same lazy-getter pattern (low risk, needs updating every call site) or migrating to `@nestjs/bullmq`'s `BullModule.registerQueue()` (already an installed but unused dependency — the more idiomatic fix, but a bigger change touching how all four queues and their workers are wired).

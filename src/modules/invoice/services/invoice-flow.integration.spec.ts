...
function makeAdapterMock(result: SubmissionResult): MockAdapter {
  return { adapterKey: "mock", adapterName: "Mock", submit: jest.fn().mockResolvedValue(result) } as any;
}
...
async function buildService(
  ...
  invoiceData?: any,
  onInvoiceUpdate?: (data: any) => void,
): Promise<{ service: SubmissionService; events: EventEmitter2; activity: jest.Mock }> {
  const prismaMock = makePrismaMock(invoiceData ?? makeInvoice(), onInvoiceUpdate);
  ...
  const module: TestingModule = await Test.createTestingModule({
    ...
    providers: [
      ...
      { provide: MockAdapter, useValue: adapterMock },
      {
        ...
        useValue: { adapterKey: "interswitch", adapterName: "Interswitch", submit: jest.fn() },
      },
    ],
  }).compile();
  ...
  describe("Flow 1: create → queue → submit → ACCEPTED", () => {
    ...
    it("transitions invoice through SUBMITTING → ACCEPTED", async () => {
      ...
      const { service } = await buildService(
        acceptedResult,
        makeInvoice(),
        (data) => { if (data.status) statusUpdates.push(data.status); },
      );
    ...
    it("records errorCode in submission attempt", async () => {
      ...
      const module = await Test.createTestingModule({
        ...
        providers: [
          ...
          { provide: InterswitchAdapter, useValue: { adapterKey: "interswitch", submit: jest.fn() } },
        ],
      }).compile();

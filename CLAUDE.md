...
- `invoice.service.ts` — still **2,070 lines** — god service: create, validate, XML, draft, duplicate, stats, charts, sample. **Worse than a size problem:** FIRS field validation is implemented three separate times with three different rule-sets that disagree — inline in `createInvoice()`, inline in `submitDraft()` (which additionally requires buyer.tin for B2B/G2G and a non-empty line-items/positive-total check that the other two don't), and the standalone `validateInvoice()` method backing `POST /v1/invoices/validate` (missing rules the other two have). A caller can get a "valid" result from `/validate` and then have `submitDraft` reject the same data. A full split plan (phased: dashboard/analytics extraction → mapper/export helpers → validation consolidation → draft/lifecycle extraction) was written up 2026-07-06 for engineering review — the validation-consolidation phase specifically needs product/compliance sign-off on which conflicting rule wins wherever the three rule-sets disagree, since it changes which live invoices get accepted/rejected. Not yet started.

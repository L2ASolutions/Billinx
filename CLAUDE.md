...
45 models, 23 enums (added `ProductItemType` and `PaymentStatusType` 2026-07-18). Key ones (many newer models — Client, InventoryMovement, Notification, VatEntry, ReminderRule, CreditNote, etc. — omitted here for brevity):
...
| `Invoice` | Core FIRS invoice with full financial + party data. `paymentStatus` is `PaymentStatusType` (`PENDING\|PAID\|PARTIAL`, NOT NULL, default `PENDING` — was a free-text nullable column until 2026-07-18). `lastNrsStatusUpdateAt`/`lastNrsStatusUpdateSuccess` track the outcome of the most recent NRS `UpdateStatus` call (see `submission` module's UpdateStatus queue). |
...
**NEW, UNRESOLVED — found 2026-07-18 during the UI-cleanup audit (PR `chore/ui-cleanup-admin-actions`), needs its own scoped PR with live-submission testing:**
- **The New Invoice dashboard form (`apps/web/app/(dashboard)/invoices/new/page.tsx`) builds a *flat* line-item payload shape that does not match the *canonical nested* shape `InterswitchAdapter`'s private `buildPayload()` (`src/modules/submission/adapters/interswitch/interswitch.adapter.ts:~811-844`) and `XmlInvoiceBuilder` (`src/modules/invoice/services/xml-invoice.builder.ts:203-235`) actually read at submission/export time.** The form's `buildPayload()` (lines 911-924) sends `{quantity, unitPrice, priceUnit, taxCode, hsnCode, isicCode}` per line item — no `productCategory`/`serviceCategory` at all, no `price` nesting. The adapter reads `item.invoicedQuantity`, `item.price.priceAmount`, `item.price.baseQuantity`, `item.hsnCode`+`item.productCategory` (or `item.isicCode`+`item.serviceCategory` for services) — none of which line up with what the form actually sends beyond the bare `hsnCode`/`isicCode` strings. `InvoiceService.createInvoice()` stores `request.lineItems` completely verbatim (no transformation — confirmed at `invoice.service.ts:160`), so whatever shape the client sends is exactly what a submission worker later hands to the adapter.
  - **This was found while investigating why `baseQuantity` (`packages/types/canonical-invoice.ts:70`) is missing from the New Invoice form** — adding just that one field would not have "round-tripped to the NRS payload" as requested, because the surrounding shape mismatch means the adapter can't read `quantity`/`unitPrice`/`productCategory` from dashboard-created invoices either, not just `baseQuantity`. Fix 5 (adding `baseQuantity` to the form) was explicitly held out of the UI-cleanup PR for this reason — don't add it back without fixing the shape mismatch first, or it'll look done without doing anything.
  - **Empirically confirmed against the dev DB**: 0 of the last 25 real invoices have the flat shape — every one is canonical, meaning they were created via seed scripts or a direct API call in the canonical shape, not through this form as currently coded.
  - CREATE-context validation (`InvoiceValidationService.validateThrowing`, `context === 'CREATE'`) does **not** enforce line-item classification/pricing shape (that's SUBMIT-only, see `invoice-validation.service.ts:384-448`), so `POST /v1/invoices/dashboard` itself won't reject a flat-shape body at creation time — the mismatch only bites once a submission worker actually calls the adapter (or the dashboard's "Submit" action runs SUBMIT-context validation, which requires `productCategory` the form never sends).
  - Scope for the follow-up PR: decide whether to (a) restructure the form's `buildPayload()` to emit the canonical nested shape directly, or (b) add a server-side normalisation step in `createInvoice()`/`submitDraft()` that maps the dashboard's flat shape into canonical before storing/submitting. Either way this touches real invoice-creation/submission behaviour and needs live-submission testing (ideally against the NRS sandbox, not just `MockAdapter`, since `MockAdapter` doesn't validate field structure the way `InterswitchAdapter` does — that's very likely why this has gone unnoticed in dev).

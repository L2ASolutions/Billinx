...
- 152+ PRs merged to main (through PR #188 as of 2026-07-09; `fix/nrs-schema-alignment` pending merge)
- 954 tests passing, 60 suites, 45 DB models, 23 enums — up from 863/56 (2026-07-09) via several PRs since (OAuth/prettier/lint work not separately itemised here) plus `fix/nrs-schema-alignment` (2026-07-18): new `payment.service.spec.ts` (18 tests — this file, `src/modules/invoice/services/payment.service.ts`, had **zero** prior coverage), `update-status.worker.spec.ts` (5 tests), `interswitch.adapter.spec.ts` rewritten for OAuth (53 tests), plus new/updated cases in `invoice-validation.service.spec.ts`, `tenant.service.spec.ts`, and `product-catalog.service.spec.ts` for the new NRS-schema rules.
...
- ~~#7 QR code storage~~ — **confirmed working**, no change needed: `SubmissionService.handleSuccess()` persists `result.qrCodeBase64` (from the adapter's NRS response) to `Invoice.qrCodeBase64` on acceptance. **PDF rendering not done — no PDF generation exists anywhere in this codebase** (backend or frontend) to attach it to; building one from scratch (library choice, invoice template, new endpoint) is out of scope for a schema-alignment PR per the decision that authorized this work. Needs its own follow-up PR. Related, found along the way: the frontend's "Download PDF" button (`apps/web/app/(dashboard)/invoices/[id]/page.tsx`, `handleDownloadPdf`) actually downloads the **XML** export, not a PDF — mislabeled, not a QR/PDF issue itself, but the same follow-up PR should fix the copy or the behaviour.
...
13. **New, found 2026-07-10 while building the top-nav user avatar dropdown (not fixed — frontend-only workaround applied instead, see below):** the JWT access token issued at login (`issueAccessToken` in `src/modules/user/services/user.service.ts`) never includes a `name` claim — only `sub`, `tenantId`, `email`, `roles`, `role`, `environment`, `tier`. The frontend (`apps/web/lib/auth.tsx`) decodes the token and sets `AuthContextValue.user.name = decoded.name ?? decoded.email` — since `name` is never present, `user.name` silently resolves to the **email address** for every user, always, not just in edge cases. This was previously masked because the sidebar's old bottom user-profile section (removed in this same change, see below) sourced its display name from a *different* place (`useUserProfile()`'s `profile.fullName`/`firstName`+`lastName`, populated via a separate `/users/me`-style call from `app/(dashboard)/layout.tsx`), not from the JWT-derived `user.name` at all. Verified live: created a throwaway test tenant/user directly via Prisma (cleaned up afterward, no trace left in the dev DB) and confirmed in a real browser session that `user.name` renders as `ui-verify@example.test`, not `Test Verifier`. Worked around on the frontend for the new avatar dropdown by sourcing the display name from `profile` the same way the old sidebar did (`profile?.fullName ?? (profile?.firstName && profile?.lastName ? ... : undefined) ?? user?.name ?? ''`), not by touching the JWT/backend. The backend gap itself is unfixed — anywhere else in the app that reads `user.name` directly from auth context (if any) would have the same silent email-instead-of-name bug.
...
- The invoice detail page's "Download PDF" button (`apps/web/app/(dashboard)/invoices/[id]/page.tsx`, `handleDownloadPdf`) actually downloads the XML export — there is no PDF generation anywhere in the codebase yet (found 2026-07-18 while scoping the QR-code-on-PDF follow-up, see Open Issues #7 above).

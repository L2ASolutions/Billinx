...
export class HealthController {
  ...
  @Get("health")
  @ApiOperation({ summary: "Health check with latency metrics — used by load balancer" })
  async health() {
    const dbStart = Date.now();
    const database = await this.prisma.$queryRaw`SELECT 1`
      .then(() => ({ status: "connected", latencyMs: Date.now() - dbStart }))
      .catch(() => ({ status: "unavailable", latencyMs: Date.now() - dbStart }));
    ...
    const redisResult = await this.redis.client
      ...
      .catch(() => ({ status: "unavailable", latencyMs: Date.now() - redisStart }));
    ...
    const isHealthy = database.status === "connected" && redisResult.status === "connected";

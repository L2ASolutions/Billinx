...
- **Known residual limitation, deliberately not addressed by this fix (shape-only, per explicit scope decision):** the New Invoice form never collects `productCategory` or `serviceCategory` at all — those fields don't exist anywhere in the form's `LineItem` interface or `buildPayload()` output (`apps/web/app/(dashboard)/invoices/new/page.tsx:16-27,911-924`). `normaliseLineItems()` fixes the *shape* (so a value, if present, ends up where the adapter/validator look for it) but cannot invent data the form never sent. **A dashboard-created invoice will still hit `MISSING_PRODUCT_CLASSIFICATION` (PRODUCT items) or `MISSING_SERVICE_CLASSIFICATION` (SERVICE items) at `submitDraft()`'s SUBMIT-context validation** until the form is updated to actually collect these two fields — tracked as a follow-up, scope TBD (add form fields vs. a defensive default such as falling back to `description`, which was explicitly rejected for this PR since defaulting a compliance classification code to free-text is a business decision, not a shape fix).
